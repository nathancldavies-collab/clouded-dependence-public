...
from clouded_deps.directories import DATA_DIR, TESTS_DIR
...
# ---------------------------------------------------------------------------
def build_test_sample(merged_path: Path, classified_path: Path) -> pd.DataFrame:
    """
    ...
    unspec_cloud = classified[
        classified["is_cloud"] & (classified["platform_stage1"].isin(["Unspecified"]))
    ]
    ...
    clear_noncloud = noncloud[
        ~noncloud["description"].str.contains(no_cloud_terms, na=False, regex=True)
    ]
...
def analyze_results(df: pd.DataFrame):
    ...
    for prompt_name in ["concise", "extended"]:
        ...
        print(
            f"  Overall: {cloud_count}/{len(df)} classified as cloud ({cloud_count / len(df) * 100:.1f}%)"
        )
            ...
            if cat == "clear_cloud":
                ...
                if fn > 0:
                    ...
                    for _, r in misses.head(5).iterrows():
                        print(
                            f"      FN: ${r['dollars'] / 1e6:.1f}M | {str(r['description'])[:80]}"
                        )
                ...
                if fp > 0:
                    ...
                    for _, r in fps.head(5).iterrows():
                        print(
                            f"      FP: ${r['dollars'] / 1e6:.1f}M | {str(r['description'])[:80]}"
                        )
                        print(f"          Evidence: {r[f'{prefix}evidence']}")
                ...
                has_platform = subset[
                    ~subset[plat_col].isin(["Unspecified", "N/A", "error"])
                ]
                ...
                for cls, count in subset[cls_col].value_counts().items():
                    ...
                    print(
                        f"      {cls:30s}: {count:>3} records, ${dollars / 1e6:>8.1f}M"
                    )
    ...
    if disagree > 0:
        ...
        for _, r in conflicts.iterrows():
            ...
            print(
                f"    [{r['test_category']:20s}] ${r['dollars'] / 1e6:>7.1f}M | {desc}"
            )
    ...
    if len(both_cloud) > 0:
        plat_agree = (
            both_cloud["concise_platform"] == both_cloud["extended_platform"]
        ).sum()
        print(
...
def main() -> None:
    ...
    classified_path = (
        DATA_DIR / "02_processed" / "03_classified" / "classified_dataset.csv"
    )
    ...
    # Save results
    output_dir = TESTS_DIR / "prompt_test_results"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / "prompt_comparison_results.csv"
