...
CACHE_DIR = DATA_DIR / "02_processed" / "03_classified" / "llm_cache"
MANIFEST_PATH = CACHE_DIR / "batch_manifest.json"
...
def download_chunk(batch_entry: dict[str, Any], client: Any) -> pd.DataFrame | None:
    """Download results for a single completed chunk."""
    from clouded_deps.pipeline import cloud_classification as cc
    ...
    # Save per-chunk file
    chunk_path = CACHE_DIR / f"chunk_{chunk_num:02d}_results.csv"
...
def submit_chunk(
    chunk_entry: dict[str, Any], client: Any, merged_df: pd.DataFrame
) -> dict[str, Any]:
...
def merge_all_chunks(manifest: dict[str, Any]) -> pd.DataFrame | None:
    """Merge all chunk CSV files into final results."""
    total_chunks = len(manifest["batches"])
    all_dfs = []

    for i in range(1, total_chunks + 1):
        chunk_path = CACHE_DIR / f"chunk_{i:02d}_results.csv"
...
def patch_chunk_with_retry(client: Any, cache_dir: Path, chunk_num: int) -> bool:
    """Download retry batch results and patch the specified chunk."""
    retry_path = cache_dir / f"retry_chunk_{chunk_num:02d}.json"
    ...
    if batch.processing_status != "ended":
        print(
            f"  Retry batch {batch_id}: {batch.processing_status} "
    ...
    print(
        f"  Downloading retry results ({batch.request_counts.succeeded:,} succeeded, "
...
def main() -> None:
    ...
    for batch_entry in manifest["batches"]:
        chunk_path = CACHE_DIR / f"chunk_{batch_entry['chunk']:02d}_results.csv"
        ...
        if batch.processing_status == "ended":
            download_chunk(batch_entry, client)
    ...
    retry_files = [
        ...
        if f.name.startswith("retry_chunk_") and f.suffix == ".json"
    ]
    if retry_files:
        print("\n" + "=" * 60)
    ...
    if pending:
        ...
        for chunk_entry in pending:
            try:
                new_batch = submit_chunk(chunk_entry, client, merged_df)
                ...
                if "429" in error_msg or "rate" in error_msg.lower():
                    print(
                        f"  Rate limited. Stopping submissions. {len(pending) - submitted_this_run - len(still_pending)} chunks deferred."
                    )
    ...
    total_expected_chunks = (
        manifest["total_records"] + manifest["chunk_size"] - 1
    ) // manifest["chunk_size"]
    downloaded = sum(
        1
        for i in range(1, total_expected_chunks + 1)
        if (CACHE_DIR / f"chunk_{i:02d}_results.csv").exists()
    )
