...
def calculate_contractor_hhi(merged_df: pd.DataFrame) -> dict[str, Any]:
    ...
    for i, (entity, row) in enumerate(contractor_spending.head(20).iterrows(), 1):
        ...
        print(
            f"    {i:2d}. {row['name'][:45]:45s} ${row['dollars']:>14,.2f} ({share:5.2f}%) [{row['records']:,} records]"
        )

    return result


def compare_baseline_approaches(
    primes_df: pd.DataFrame, merged_df: pd.DataFrame
) -> dict[str, Any]:
    """
    ...
    old_spending = primes_df.groupby("resolved_entity").agg(
        dollars=("Total Dollars Obligated", "sum"), name=("Contractor Name", "first")
    )
    ...
    print(f"  {'-' * 75}")
    print(
        f"  {'HHI':<30s} {old_hhi:>15,.1f} {new_result['hhi']:>15,.1f} {hhi_change:>+15,.1f}"
    )
    print(
        f"  {'C4 (%)':<30s} {old_c4:>15.2f} {new_result['c4']:>15.2f} {c4_change:>+15.2f}"
    )
    print(
        f"  {'N contractors':<30s} {old_n:>15,} {new_result['n_contractors']:>15,} {n_change:>+15,}"
    )
    print(
        f"  {'Classification':<30s} {classify_hhi(old_hhi):>15s} {new_result['classification']:>15s}"
    )

    if hhi_change < 0:
        print(f"\n  Baseline HHI DECREASED by {abs(hhi_change):,.1f} points.")
        print("  Market appears LESS concentrated when subaward flows are visible.")
        print(
            f"  This is because {n_change:+,} additional entities appear in the merged data."
        )
    elif hhi_change > 0:
