...
def filter_primes(
    primes_df: pd.DataFrame, year_min: int = 2017, year_max: int = 2024
    ...
    print(
        f"    {after_date_count:,} contracts ({after_date_count / baseline_count * 100:.1f}%), "
    ...
    print(
        f"    {after_psc_count:,} contracts ({after_psc_count / baseline_count * 100:.1f}%), "
    ...
    df_final = df_psc[
        df_psc["keyword_classification"].isin(keep_classifications)
    ].copy()
    ...
    print(
        f"    {after_kw_count:,} contracts ({after_kw_count / baseline_count * 100:.1f}%), "
    ...
    print(
        f"    Total excluded: {total_excluded:,} contracts ({total_excluded / baseline_count * 100:.1f}%), "
        f"${total_dollars_excluded / 1e9:.1f}B ({total_dollars_excluded / baseline_dollars * 100:.1f}%)"
    )

    # Drop temporary columns
    df_final = df_final.drop(
        columns=["psc_prefix", "is_hardware_psc", "keyword_classification"],
        errors="ignore",
    )
