...
def expand_platform_mentions(
    df: pd.DataFrame,
    mentions_col: str = "platform_mentions",
    base_platform_col: str = "final_platform",
) -> pd.DataFrame:
    """
...
def attribute_direct_entities(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _match_platform_entity(name: object) -> str | None:
        if pd.isna(name):
            return None
    ...
    def _check_name_affiliation(name: object) -> str | None:
        if pd.isna(name):
            return None
    ...
    for rtype in ["prime_no_subs", "prime_retained", "subcontract"]:
        ...
        if len(subset) > 0:
            print(
                f"    {rtype:20s}: {len(subset):,} records, ${subset['dollars'].sum():,.2f}"
            )

    if direct_count > 0:
        print("\n  Direct entity breakdown:")
        for platform, group in df[df["platform_phase1"].notna()].groupby(
            "platform_phase1"
        ):
            print(
                f"    {platform:20s}: {len(group):,} records, ${group['dollars'].sum():,.2f}"
            )
...
def attribute_from_descriptions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[unattributed_cloud, "platform_phase2"] = df.loc[
        unattributed_cloud, "platform_stage1"
    ]
...
def build_contractor_patterns(
    ...
    print(
        f"Phase 3: Pattern Matching (min_evidence={min_evidence}, "
    ...
    attributed = df[
        df["is_cloud"] & df["platform_phase2"].notna() & (df["dollars"] > 0)
    ].copy()
    ...
    contractor_platforms = (
        attributed.groupby(["contractor", "platform_phase2"])
        .agg(evidence_count=("dollars", "count"), evidence_dollars=("dollars", "sum"))
        .reset_index()
    )
    ...
    for contractor in contractor_platforms["contractor"].unique():
        ...
        if consistency >= consistency_threshold:
            contractor_name = df.loc[
                df["contractor"] == contractor, "contractor_name"
            ].iloc[0]
            patterns.append(
                {
...
def apply_patterns(df: pd.DataFrame, patterns_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[unattributed_mask, "platform_phase2"] = df.loc[
        unattributed_mask, "contractor"
    ].map(pattern_lookup)
...
def synthesize_final_platform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(
        f"  Attributed to platform: {len(platform_attributed):,} "
        f"({len(platform_attributed) / total_cloud * 100:.1f}%), "
        f"${platform_attributed['dollars'].sum() / 1e6:.1f}M"
    )
    print(
        f"  At contractor level: {len(contractor_attributed):,} "
    ...
    for method, group in df[df["is_cloud"]].groupby("attribution_method"):
        ...
        print(f"    {method!s:35s}: {n:>7,} records, ${dollars / 1e6:>9.1f}M")
...
def calculate_platform_hhi(
    df: pd.DataFrame,
    platform_column: str = "final_platform",
    dollars_column: str = "dollars",
) -> dict[str, Any]:
    """
    ...
    platform_spending = (
        cloud_df.groupby(platform_column)[dollars_column]
        .sum()
    ...
    for platform, share in platform_shares.head(15).items():
        ...
        print(
            f"    {str(platform)[:30]:30s}: {share:6.2f}% (${dollars:>14,.2f}, {count:,} records)"
        )
    ...
    if "attribution_method" in cloud_df.columns:
        print("\n  Attribution method breakdown:")
        method_summary = (
            cloud_df.groupby("attribution_method")
            .agg(records=(dollars_column, "count"), dollars=(dollars_column, "sum"))
            .sort_values("dollars", ascending=False)
        )
        for method, row in method_summary.iterrows():
            pct = row["dollars"] / total_cloud_dollars * 100
            print(
                f"    {method!s:35s}: {row['records']:>7,} records, "
...
def run_stage2(
    df: pd.DataFrame, min_evidence: int = 5, consistency_threshold: float = 0.90
    ...
    if len(contractor_patterns) > 0 and len(contractor_patterns_dollars) > 0:
        ...
        diff = merged[
            merged["learned_platform_count"] != merged["learned_platform_dollars"]
        ]
        if len(diff) > 0:
            print(
...
def run_attribution_pipeline(
    ...
    from clouded_deps.pipeline import cloud_classification as cls_mod
...
def main() -> tuple[pd.DataFrame, dict[str, Any]] | None:
    ...
    classified_path = output_dir / "classified_dataset.csv"
    ...
    if classified_path.exists():
        ...
        print(
            "WARNING: No classified dataset found. Using merged dataset (RegEx-only mode)."
        )
    ...
    output_path = output_dir / "attributed_dataset.csv"
