...
def safe_read_csv(
    path: str, usecols: list[str] | None = None, chunksize: int = 200_000
) -> pd.DataFrame:
    """
...
def compile_patterns() -> dict:
    ...
    compiled = {"platforms": {}, "generic_cloud": [], "explicit_hyperscaler": {}}
    for platform, data in PLATFORM_PATTERNS.items():
        ...
        if "keywords" in data:
            ...
            all_patterns = data.get("brand_names", []) + data.get(
                "specific_services", []
            )
    ...
    compiled["generic_cloud"] = [
        re.compile(p, re.IGNORECASE) for p in GENERIC_CLOUD_KEYWORDS
    ]
...
def classify_cloud_regex(merged_df: pd.DataFrame, patterns: dict) -> pd.DataFrame:
    ...
    def _explicit_hyperscaler_mentions(
        text_a: str, text_b: str | None = None
    ) -> list[str]:
        mentions = set()
    ...
    def _is_cloud(description: object, contractor_name: object) -> bool:
    ...
    def _detect_platforms(
        description: object, contractor_name: object
        ...
        concrete = [p for p in platform_matches if p in CONCRETE_PLATFORMS]
        if len(concrete) >= 2:
    ...
    platform_results = df.apply(
        lambda r: _detect_platforms(r.get("description"), r.get("contractor_name")),
        axis=1,
    )
    ...
    df["explicit_hyperscaler_mentions"] = df.apply(
        lambda r: _explicit_hyperscaler_mentions(
            ...
            str(r.get("contractor_name")).lower()
            if pd.notna(r.get("contractor_name"))
            else "",
        ),
        axis=1,
    )
    df["explicit_cloud_mention"] = df["explicit_hyperscaler_mentions"].apply(bool)
    ...
    print(
        f"  Cloud dollars: ${cloud_dollars:,.2f} ({cloud_dollars / total_dollars * 100:.1f}%)"
    )
    ...
    for rtype in ["prime_no_subs", "prime_retained", "subcontract"]:
        ...
        if len(subset) > 0:
            ...
            print(
                f"    {rtype:20s}: {cloud_sub:,} cloud / {len(subset):,} total "
...
def _parse_llm_response(text: str) -> dict[str, str]:
    ...
    try:
        ...
        cls_match = re.search(
            r'"classification"\s*:\s*"(non-cloud|cloud-dependent|cloud-infrastructure)"',
            text,
        )
...
def _load_llm_checkpoint(cache_dir: str | Path) -> tuple[list[dict], int]:
    """Load existing LLM checkpoint if available. Returns (results_list, last_completed_index)."""
    cache_dir = Path(cache_dir)
    progress_path = cache_dir / "llm_progress.json"
    ...
    if progress.get("status") == "complete":
        ...
        final_path = cache_dir / "llm_results_final.csv"
    ...
    batch_num = 1
    while True:
        batch_path = cache_dir / f"llm_results_batch_{batch_num:03d}.csv"
...
def _save_llm_checkpoint(
    results: list[dict], cache_dir: str | Path, batch_label: str, total_records: int
) -> None:
    ...
    # Save batch file
    batch_path = cache_dir / f"llm_results_batch_{batch_label}.csv"
    ...
    with (cache_dir / "llm_progress.json").open("w") as f:
        json.dump(progress, f, indent=2)


def classify_cloud_llm(
    df: pd.DataFrame,
    api_key: str,
    cache_dir: str | Path,
    ...
    print(
        f"  Estimated time: {estimate['estimated_minutes']:.0f} minutes "
    ...
    for i in tqdm(
        range(start_idx, n), desc="LLM Classification", initial=start_idx, total=n
    ):
...
def load_llm_results(cache_dir: str | Path | None) -> pd.DataFrame | None:
    """
    ...
    try:
        return safe_read_csv(
            final_path, usecols=["classification", "platform", "confidence", "evidence"]
        )
    except Exception as e:
...
def _build_batch_requests(
    df: pd.DataFrame, prompt_fn: Callable[[str, str], str] | None = None
) -> list[dict]:
    """
...
def submit_llm_batch(
    ...
    try:
        ...
        raise ImportError(
            "The 'anthropic' package is required. Install: pip3 install anthropic"
        )
    ...
    metadata_path = cache_dir / f"batch_{batch_id}.json"
...
def check_batch_status(
    batch_id: str, api_key: str, cache_dir: str | Path | None = None
) -> dict:
    """
    ...
    if cache_dir:
        metadata_path = Path(cache_dir) / f"batch_{batch_id}.json"
...
def download_batch_results(
    batch_id: str, api_key: str, cache_dir: str | Path, n_records: int
) -> pd.DataFrame | None:
    """
...
def synthesize_classification(
    ...
    noncloud_override_confidence: str = "high",
) -> pd.DataFrame:
    """
    ...
    has_llm = (
        "llm_classification" in df.columns and df["llm_classification"].notna().any()
    )
    ...
    if not has_llm:
        ...
        df.loc[df["is_cloud"] & df["platform_stage1"].isna(), "platform_stage1"] = (
            "Unspecified"
        )
        ...
        df["platform_mentions"] = (
            df["regex_platforms_list"]
            if "regex_platforms_list" in df.columns
            else [[]] * len(df)
        )
    else:
        ...
        for i in range(n):
            ...
            if llm_has_error:
                ...
                plat_s1.append(
                    regex_plat
                    if regex_cloud and regex_plat
                    else ("Unspecified" if regex_cloud else "N/A")
                )
                conf.append("low")
            ...
            if regex_cloud and llm_is_cloud:
                ...
                best_plat = (
                    llm_plat if llm_plat not in ("N/A", None, "error") else regex_plat
                )
            ...
            if not regex_cloud and llm_is_cloud:
                ...
                plat_s1.append(
                    llm_plat
                    if llm_plat not in ("N/A", None, "error")
                    else "Unspecified"
                )
                conf.append("medium")
    ...
    print(
        f"  Cloud dollars: ${cloud_dollars / 1e9:.2f}B ({cloud_dollars / total_dollars * 100:.1f}%)"
    )
    ...
    for plat, count in cloud_plats.value_counts().items():
        dollars = df.loc[
            (df["is_cloud"]) & (df["platform_stage1"] == plat), "dollars"
        ].sum()
        print(f"    {plat!s:25s}: {count:>6,} records, ${dollars / 1e6:>9.1f}M")
...
def validate_llm_conflicts(df: pd.DataFrame) -> None:
    ...
    """
    if (
        "llm_classification" not in df.columns
        or "explicit_cloud_mention" not in df.columns
    ):
        return
    subset = df[df["explicit_cloud_mention"]]
    if len(subset) == 0:
        return
    noncloud = subset[
        ~subset["llm_classification"].isin(["cloud-dependent", "cloud-infrastructure"])
    ]
    if len(noncloud) == 0:
        print(
...
def run_stage1(
    ...
    cached_llm = load_llm_results(cache_dir) if cache_dir else None

    if cached_llm is not None:
        print(f"\n  Loading cached LLM results ({len(cached_llm):,} records)")
        if len(cached_llm) != len(df):
            print(
                "  WARNING: Cached LLM results length does not match current dataset."
            )
        ...
        df = classify_cloud_llm(
            df, api_key, cache_dir, skip_confirmation=skip_confirmation
        )
    else:
...
def main() -> pd.DataFrame | None:
    ...
    merged_path = DATA_DIR / "02_processed" / "02_merged" / "merged_dataset.csv"
    output_dir = DATA_DIR / "02_processed" / "03_classified"
    ...
    output_path = output_dir / "classified_dataset.csv"
