...
def load_primes(filepath: str) -> pd.DataFrame:
    ...
    print(
        f"  Unique contractors (by resolved entity): {df['resolved_entity'].nunique():,}"
    )
    print(
        f"  Primes with subs (Subaward Count > 0): {(df['Subaward Count'] > 0).sum():,}"
    )

    return df


def dedupe_primes_by_award(
    primes_df: pd.DataFrame, dollars_agg: str = "max"
) -> pd.DataFrame:
    """
    ...
    def _first_non_null(s: pd.Series) -> str | None:
        s = s.dropna()
        return s.iloc[0] if len(s) > 0 else None

    def _longest_text(s: pd.Series) -> str | None:
    ...
    text_cols_first = [
        ...
        "Contractor Name",
        "Contractor UEI",
        "Contractor Parent UEI",
        "Award Type",
        "NAICS Code",
        "PSC Code",
        "Funding Department/Agency",
        "Funding Parent Office",
        "Funding Office",
        "Funding Office City",
        "Funding Office State",
        "Awarding Department/Agency",
        "Awarding Parent Office",
        "Awarding Office",
        "Price Type",
        "Type of Set Aside",
        "Extent Competed",
    ...
    deduped["resolved_entity"] = deduped["Contractor Parent UEI"].fillna(
        deduped["Contractor UEI"]
    )
    if "Effective Date" in deduped.columns:
        deduped["Effective Date"] = pd.to_datetime(
            deduped["Effective Date"], errors="coerce"
        )
...
def extract_prime_award_id(subaward_id: str, prime_id_set: set) -> str | None:
    """
...
def create_merged_dataset(
    primes_df: pd.DataFrame, subs_df: pd.DataFrame, return_unlinked: bool = False
    ...
    _has_parent = _has_parent[
        _has_parent["Contractor UEI"] != _has_parent["Contractor Parent UEI"]
    ]
    uei_to_parent = (
        _has_parent.drop_duplicates("Contractor UEI")
        .set_index("Contractor UEI")["Contractor Parent UEI"]
        .to_dict()
    )
    ...
    if len(unlinked_df) > 0:
        ...
        print(
            f"  WARNING: {unlinked:,} subawards could not be linked to a prime "
    ...
    for _, prime_row in primes_df.iterrows():
        ...
        if award_id in primes_with_subs:
            ...
            for _, sub_row in sub_group.iterrows():
                ...
                sub_entity = (
                    uei_to_parent.get(sub_uei, sub_uei)
                    if pd.notna(sub_uei)
                    else sub_uei
                )
    ...
    if scaled_count > 0:
        print(
            f"  NOTE: {scaled_count:,} primes had sub totals exceeding prime amount "
...
def verify_merged_dataset(
    ...
    subs_df: pd.DataFrame,  # noqa: ARG001  (kept for call-site symmetry)
) -> bool:
    """
    ...
    if (retained["dollars"] < 0).any():
        ...
        print("  This means subaward totals exceed prime amounts for those contracts")
...
def print_merge_summary(merged_df: pd.DataFrame):
    ...
    for rtype in ["prime_no_subs", "prime_retained", "subcontract"]:
        ...
        print(
            f"  {rtype:20s}: {count:>8,} records ({pct_records:5.1f}%), "
    ...
    contractor_totals = (
        merged_df.groupby(["contractor", "contractor_name"])
        .agg(dollars=("dollars", "sum"), records=("dollars", "count"))
    ...
    for i, ((entity, name), row) in enumerate(contractor_totals.head(15).iterrows(), 1):
        ...
        print(
            f"  {i:2d}. {name[:45]:45s} ${row['dollars']:>14,.2f} ({pct:5.2f}%) [{row['records']:,} records]"
        )

    # Fiscal year breakdown
    print("\nBy fiscal year:")
    fy_totals = (
        merged_df.groupby("fiscal_year")
        .agg(dollars=("dollars", "sum"), records=("dollars", "count"))
        .sort_index()
    )
    for fy, row in fy_totals.iterrows():
        ...
        print(
            f"  FY{fy_display}: {row['records']:>8,} records, ${row['dollars']:>14,.2f}"
        )
...
def main() -> None:
    ...
    output_dir = DATA_DIR / "02_processed" / "02_merged"
