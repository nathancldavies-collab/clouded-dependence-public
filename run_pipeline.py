...
LLM classification:
  - Pass --run-llm to enable LLM classification. ANTHROPIC_API_KEY must be set;
    otherwise the pipeline exits immediately.
  - Without --run-llm, the pipeline runs in RegEx-only mode.
...
Run from project root:
    python3 run_pipeline.py [--run-llm]
"""
...
def run_full_pipeline(run_llm: bool = False):
    ...
    if run_llm and not os.environ.get("ANTHROPIC_API_KEY"):
        print(
            "FATAL: --run-llm requires ANTHROPIC_API_KEY to be set in the environment."
        )
        sys.exit(1)

    start_time = time.time()

    def _calc_hhi_from_group(
        df: pd.DataFrame, group_col: str, dollars_col: str = "dollars"
    ...
    print("=" * 80)
    print(f"\nProject root: {ROOT}")
    print("Methodology: Filter -> Merge primes+subs -> Classify -> Attribute -> HHI")

    # --- Paths ---
    raw_dir = DATA_DIR / "01_raw"
    processed_dir = DATA_DIR / "02_processed"
    filtered_dir = processed_dir / "01_filtered"
    merged_dir = processed_dir / "02_merged"
    classified_dir = processed_dir / "03_classified"
    llm_cache_dir = classified_dir / "llm_cache"

    prime_path = raw_dir / "prime_awards.csv"
    sub_path = raw_dir / "subawards.csv"
    filtered_path = filtered_dir / "prime_services_filtered.csv"
    merged_path = merged_dir / "merged_dataset.csv"
    classified_path = classified_dir / "classified_dataset.csv"
    attributed_path = classified_dir / "attributed_dataset.csv"
    unlinked_path = merged_dir / "unlinked_subawards.csv"

    for d in [filtered_dir, merged_dir, classified_dir]:
        d.mkdir(parents=True, exist_ok=True)

    # --- LLM configuration ---
    api_key = os.environ.get("ANTHROPIC_API_KEY") if run_llm else None
    has_llm_cache = (llm_cache_dir / "llm_progress.json").exists()
    ...
    if run_llm:
        print("\n  LLM classification: enabled (--run-llm)")
        if has_llm_cache:
            print("  LLM cache: found (will reuse cached results)")
        else:
            print("  LLM cache: not found (will run LLM classification)")
    else:
        print(
            "\n  LLM classification: disabled (RegEx-only mode; pass --run-llm to enable)"
        )
    if freeze_merged:
        print("  Freeze merged dataset: enabled (to preserve LLM alignment)")

    # =========================================================================
    # STAGE 0A-0C: LOAD/FILTER/MERGE (optionally skipped when freezing)
    # =========================================================================
    if freeze_merged and merged_path.exists() and filtered_path.exists():
        print("\n\n" + "#" * 80)
        ...
        primes_df, filter_summary = filter_primes(
            primes_raw, year_min=2017, year_max=2024
        )
        primes_df = dedupe_primes_by_award(primes_df, dollars_agg="max")
        filter_summary["after_keyword_count"] = len(primes_df)
        filter_summary["after_keyword_dollars"] = primes_df[
        ...
        merged_df, unlinked_df = create_merged_dataset(
            primes_df, subs_df, return_unlinked=True
        )
    ...
    has_llm = (
        "llm_classification" in classified_df.columns
        and classified_df["llm_classification"].notna().any()
    )
    ...
    attributed_df, platform_result = run_stage2(
        classified_df, min_evidence=5, consistency_threshold=0.90
    )
    ...
    if classified_df_strict is not None:
        ...
        print("#" * 80)
        _, platform_result_strict = run_stage2(
            classified_df_strict, min_evidence=5, consistency_threshold=0.90
        )
    ...
    infra_platform_hhi, _ = _calc_hhi_from_group(
        infra_expanded[infra_expanded["is_cloud"]],
        "platform_for_analysis",
        dollars_col="dollars_split",
    )
    ...
    print(
        f"  {'Classification':<35s} {classify_hhi(baseline_hhi):>15s} "
        f"{platform_result['classification']:>15s}"
    )
    print(
        f"  {'C4 (%)':<35s} {baseline_comparison['new_c4']:>15.2f} "
    ...
    if hhi_baseline_change < 0:
        print("  -> More entities visible after merge -> baseline DECREASES")
        print("  -> Gap between baseline and platform view is LARGER")
        print("  -> Finding is STRONGER with correct methodology")
    ...
    cloud_df = attributed_df[attributed_df["is_cloud"]]
    cloud_total = expanded_df[expanded_df["is_cloud"]]["dollars_split"].sum()
    ...
    for platform, patterns in hyperscaler_patterns.items():
        ...
        contractor_mask = (
            attributed_df["contractor_name"]
            .str.upper()
            .str.contains("|".join(p.upper() for p in patterns), na=False)
        )
        ...
        print(
            f"  {platform:<15s} {baseline_pct:>14.2f}% {platform_pct:>14.2f}% {change_str:>10s}"
        )

    print("\n  'As Contractor' = share of total IT market as a direct vendor")
    print(
        "  'As Platform'   = share of cloud market where spending depends on their platform"
    )
    print("  'Change'        = percentage increase from contractor to platform share")
    ...
    temporal_cloud = temporal_df[temporal_df["is_cloud"]]
    temporal_expanded = expanded_df[
        expanded_df["fiscal_year"].between(2017, 2024)
    ].copy()
    temporal_cloud_expanded = temporal_expanded[temporal_expanded["is_cloud"]]
    ...
    for start_fy in range(2017, 2025 - WINDOW + 1):
        ...
        w_cloud = temporal_cloud[
            temporal_cloud["fiscal_year"].between(start_fy, end_fy)
        ]
        ...
        platform_shares = (
            w_cloud_expanded.groupby("platform_for_analysis")["dollars_split"].sum()
            / cloud_d
            * 100
        )
    ...
    for w in windows:
        print(
            f"  {w['label']:>12s} {w['total_d'] / 1e9:>10.1f} {w['cloud_d'] / 1e9:>10.1f} "
    ...
    print(
        f"  {'Platform':>15s} {first_w['label']:>14s} {last_w['label']:>14s} {'Change':>12s}"
    )
    print(f"  {'-' * 58}")
    for p in key_platforms:
        ...
        if first_share >= 0.1:
            ...
            print(
                f"  {p:>15s} {first_share:>13.1f}% {last_share:>13.1f}% {delta:>+11.1f}pp"
            )
        elif last_share > 0:
            print(
                f"  {p:>15s} {first_share:>13.1f}% {last_share:>13.1f}% {'emerging':>12s}"
            )

    # Summary
    print("\n  Key trends:")
    print(
        f"    Cloud penetration: {first_w['cloud_pct']:.1f}% -> {last_w['cloud_pct']:.1f}% "
        f"of IT spending (+{last_w['cloud_pct'] - first_w['cloud_pct']:.1f}pp)"
    )
    print(
        f"    Concentration gap: {first_w['gap']:.1f}x -> {last_w['gap']:.1f}x "
    ...
    print(
        f"    Azure share: {first_azure:.1f}% -> {last_azure:.1f}% "
        f"({'declining' if last_azure < first_azure else 'growing'}, "
        f"but still dominant)"
    )
    combined_others = sum(
        last_w["platform_shares"].get(p, 0)
        for p in ["AWS", "Salesforce", "Oracle Cloud", "ServiceNow"]
    )
    combined_first = sum(
        first_w["platform_shares"].get(p, 0)
        for p in ["AWS", "Salesforce", "Oracle Cloud", "ServiceNow"]
    )
    print(
        f"    Challenger platforms (AWS+Salesforce+Oracle+ServiceNow): "
    ...
    cloud_records = (
        attributed_df["is_cloud"].sum() if "is_cloud" in attributed_df.columns else 0
    )
    ...
    checks = [
        (
            "Merged total = Filtered prime total (no double-counting)",
            dollar_match,
            f"${merged_total:,.2f} vs ${prime_total:,.2f}",
        ),
        (
            "Baseline HHI calculated on merged dataset",
            baseline_hhi > 0,
            f"HHI = {baseline_hhi:,.1f}",
        ),
        (
            "Cloud classification ran on ALL merged records",
            all_classified,
        ...
        ("No null final_platform values", no_nulls, f"{null_platforms} null values"),
        (
            "Concentration gap >= 2x",
            concentration_gap >= 2.0,
    ...
    print("#" * 80)
    print(
        "  - Subaward fiscal year inherits prime fiscal year (subaward dates unavailable)."
    )
    print(
        "  - Filtering primes before merge excludes subawards tied to filtered primes."
    )
    print(
        "  - Unlinked subawards are excluded from merged dataset but saved separately."
    )
    ...
    # LLM status
    has_llm = (
        "classification_source" in attributed_df.columns
        and attributed_df["classification_source"].str.contains("llm", na=False).any()
    )
    print(f"\n  Classification: {'RegEx + LLM' if has_llm else 'RegEx-only'}")

    print("\n  Key results:")
    print(
        f"    Baseline HHI (contractor-level): {baseline_hhi:,.1f} - {classify_hhi(baseline_hhi)}"
    )
    print(
        f"    Platform HHI (cloud-attributed): {platform_hhi:,.1f} - {platform_result['classification']}"
    )
    print(f"    Concentration gap:               {concentration_gap:.1f}x")
    print("\n  Output files:")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="run_llm",
        action="store_true",
        help="Run LLM classification (requires ANTHROPIC_API_KEY)",
    )
...
if __name__ == "__main__":
    args = parse_args()
    run_full_pipeline(run_llm=args.run_llm)
